...
clip_cfg = dict(          # ViT-B/32
    ...
    image_encoder=dict(
        ...
        init_cfg=dict(
            type='Pretrained',
            prefix='visual',
            checkpoint='ovdet/checkpoints/clip_vitb32.pth')
    ),
    text_encoder=dict(
...
ovd_cfg = dict(type='BaronKD',
               boxes_cache=dict(json_path='ovdet/data/coco/wusize/instances_train2017_base.json',
                                start_iter=20000,),
               use_gt=True,
...
model = dict(
    ...
    roi_head=dict(
        ...
        bbox_head=dict(
            ...
            cls_temp=50.0,
            num_words=6,
            cls_embeddings_path='ovdet/data/metadata/coco_clip_hand_craft_attn12.npy',
            bg_embedding='learn',
...
load_from = 'ovdet/checkpoints/res50_fpn_soco_star_400.pth'
